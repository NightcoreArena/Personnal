...
- **GATE QUESTIONS** (3.5 — anti-erreur Makima "est ce que makima aime denji 20 oublié", 2026-06-19) : `phrase_questions "[perso]"` a été lancé CETTE session (base fr) ET les questions à volume sont consignées dans `semrush_data` (bloc `questions_lore`). Chaque question à volume doit être alignée sur un P2 lore d'une fiche (ex : "makima aime denji" → P2 relation Makima/Denji). Une question à volume sans P2 correspondant = longue traîne perdue → réaffecter un angle lore avant validation. Ne JAMAIS sauter cette étape même si le cluster est topique pur.
...
- **Densité du mot-clé (anti-stuffing, ajouté 2026-06-20 après erreur Akeno)** : le linter compte les occurrences du nom complet du perso (dérivé du champ `cluster`, ou champ explicite `keyword`). **FAIL si le mot-clé exact apparaît ≥3× dans le P1** (cas Akeno : sujet + nom produit + ancre du lien = stuffing), WARN à 2× en P1 ou ≥4× sur la fiche. Règle : nom complet 1× en P1, ancre de maillage en nom COURT, le reste en pronoms/épithètes (LSI).
- **Cohérence des H1 (ajouté 2026-06-20)** : WARN si certains `title` du cluster portent le nom complet du perso et d'autres non (cas Akeno : 5 "Akeno" vs 1 "Akeno Himejima"). Rappel : dès qu'un perso a un nom de famille, le H1 = nom complet partout (c'est le `[Personnage]` du format, pas la franchise).
